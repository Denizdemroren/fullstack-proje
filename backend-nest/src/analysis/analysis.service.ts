...
export class AnalysisService {
  ...
  private async cloneRepository(githubUrl: string): Promise<string> {
    ...
    try {
      // 1. SHALLOW CLONE yap (büyük repolar için)
      // --depth 1: sadece son commit
      // --filter=blob:none: dosya içeriği olmadan (daha hızlı)
      // --single-branch: sadece main branch
      ...
      this.logger.log(`Shallow clone successful. Checking if repo exists at: ${repoPath}`);
      ...
      if (packageJsonPath) {
        ...
        this.logger.warn(`No package.json found in ${finalRepoPath}, skipping dependency installation`);
      }
  ...
  private async findPackageJson(dir: string): Promise<string | null> {
    ...
    if (fs.existsSync(rootPackageJson)) {
      this.logger.log(`Found package.json at root: ${rootPackageJson}`);
      return rootPackageJson;
    }
    
    // Monorepo durumu için: backend/ ve frontend/ altında ara
    const commonSubdirs = ['backend', 'frontend', 'src', 'packages', 'apps', 'backend-nest', 'frontend-react'];
    
    for (const subdir of commonSubdirs) {
      const subdirPath = path.join(dir, subdir);
      if (fs.existsSync(subdirPath)) {
        const packageJsonInSubdir = path.join(subdirPath, 'package.json');
        if (fs.existsSync(packageJsonInSubdir)) {
          this.logger.log(`Found package.json in ${subdir}: ${packageJsonInSubdir}`);
          return packageJsonInSubdir;
        }
      }
    }
    ...
    try {
      const found = await this.searchPackageJsonRecursive(dir, 0, 3);
      if (found) {
        this.logger.log(`Found package.json recursively: ${found}`);
        return found;
      }
    } catch (error) {
      this.logger.error(`Error in recursive search: ${error.message}`);
    }
    
    this.logger.error(`No package.json found in ${dir}`);
    return null;
  }

  private async searchPackageJsonRecursive(dir: string, depth: number, maxDepth: number): Promise<string | null> {
    if (depth >= maxDepth) {
      return null;
    }
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (stat.isDirectory()) {
            ...
            // Recursive search
            const found = await this.searchPackageJsonRecursive(fullPath, depth + 1, maxDepth);
            if (found) {
              return found;
            }
          }
  ...
  try {
    ...
    // 2. Tüm bağımlılıkları topla (local links dahil)
    ...
    // LOCAL LINK'leri de ekle (link:src, link:ui, etc.)
    // Eğer local link ise, o directory'nin package.json'ını bul
    const localPackages: { [key: string]: string } = {};
    
    for (const [depName, depVersion] of Object.entries(allDependencies)) {
      if (typeof depVersion === 'string' && depVersion.startsWith('link:')) {
        const localPath = depVersion.replace('link:', '');
        const localPackageJsonPath = path.join(packageDir, localPath, 'package.json');
        
        if (fs.existsSync(localPackageJsonPath)) {
          try {
            const localPkgContent = await fs.promises.readFile(localPackageJsonPath, 'utf-8');
            const localPkgJson = JSON.parse(localPkgContent);
            
            // Local package'ın dependencies'lerini ekle
            Object.entries(localPkgJson.dependencies || {}).forEach(([localName, localVersion]) => {
              localPackages[localName] = localVersion as string;
            });
            
            Object.entries(localPkgJson.devDependencies || {}).forEach(([localName, localVersion]) => {
              localPackages[localName] = localVersion as string;
            });
            
            this.logger.log(`Found local package: ${depName} -> ${localPath}`);
          } catch (error) {
            this.logger.warn(`Could not read local package ${localPackageJsonPath}: ${error.message}`);
          }
        }
      }
    }
    
    // Local package'ları da ekle
    ...
    this.logger.log(`Local packages found: ${Object.keys(localPackages).length}`);
    ...
    for (const [depName, depVersion] of Object.entries(allDependencies)) {
      // Skip local links (already processed)
      if (typeof depVersion === 'string' && depVersion.startsWith('link:')) {
        continue;
      }
